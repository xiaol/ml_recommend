...
def recall_candidates(item_extractor, user_id):
    ...
    # big img and video
    bv_dict = recall_items.recall_bigimg_video(user_id, 1000)
...
class ItemExtractor(object):
    ...
    def enumerate_recommend_strategy(self):
        ...
        logtype = {'wilson': 0, 'channel hot news': 1, 'topic collection': 4, 'slide image news': 5, 'video': 6,
...
def load(items_list, topic_num, item_extractor):
    ...
    for item_topic in items_topic:
        items_feature_dict[item_topic[0]][item_topic[1]] = item_topic[2]*4
    ...
    for item_ck in items_channel_kmeans:
        items_feature_dict[item_ck['nid']][topic_offset + channel_feature_list.index(item_ck['ch_name'])] = 1.5
