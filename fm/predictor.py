...
def predict(time_interval='10 seconds', user=-1):
    ...
    update_user_ranking_recommend(user, recommend_items_list[:500])
...
if __name__ == '__main__':
    ...
    while True:
        ...
        try:
            if elapse <= 0:
                candidate_users = etl_user_data.get_active_user(time_active='2 minutes', click_times=9)
            else:
                time_seconds = str(elapse + 1) + ' seconds'
                candidate_users = etl_user_data.get_active_user(time_active=time_seconds, click_times=9)
        except:
